                ...
                if (state == IEdkErrorCode.EDK_OK.ToInt()) {
                    ...
                    switch (eventType) {
                        ...
                        case TYPE_EMOSTATE_UPDATE:
                            ...
                            float eyesOpen = IEmoStateDLL.IS_FacialExpressionIsEyesOpen();
                            if (IEmoStateDLL.IS_FacialExpressionIsLeftWink() == 1) {
                                Log.e("FacialExpression", "Left Wink - " +  eyesOpen);
                            }
                            if (IEmoStateDLL.IS_FacialExpressionIsRightWink() == 1) {
                                Log.e("FacialExpression", "Right Wink - " +  eyesOpen);
                            }


                            float[] eyeLidState = IEmoStateDLL.IS_FacialExpressionGetEyelidState();
                            if (eyeLidState[0]== 1) {
                                Log.e("FacialExpression", "Left Eye Closed - " +  eyesOpen);
                            }
                            if (eyeLidState[1] == 1) {
                                Log.e("FacialExpression", "Right Eye Closed - " +  eyesOpen);
                            }
...
//			    	   		
    ...
    public Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            float[] eyeLidState = IEmoStateDLL.IS_FacialExpressionGetEyelidState();
            if ((eyeLidState[0]>0)||(eyeLidState[1]>0)){
                if (delegate!=null) {
                    delegate.detectedEyeLidMovement(eyeLidState);
                }
            }
